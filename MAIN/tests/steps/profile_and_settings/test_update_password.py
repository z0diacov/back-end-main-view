...
scenarios("../../features/profile_and_settings/update_password.feature")

not_decoded_jwts = tests_setup.create_not_decoded_iter_jwts("access")

@given(parsers.parse("{token}, {new_password} and {old_password}"))
def step_given_reset_password(token, new_password, old_password, data):
    data['new_password'] = None if new_password == "None" else new_password
    data['old_password'] = None if old_password == "None" else old_password
...
def step_when_reset_password(client, data, response_fixture):
    ...
    if data['token'] == None:
        response = client.put("/password", json=json_data)
    else:
        response = client.put("/password", json=json_data, headers={"Authorization": f"Bearer {data['token']}"})
