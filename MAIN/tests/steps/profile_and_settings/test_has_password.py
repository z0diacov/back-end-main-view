...
scenarios("../../features/profile_and_settings/has_password.feature")
...
def step_when_reset_password(client, data, response_fixture) -> None:
    ...
    if data['token'] == None:
        response = client.get("/me/has-password")
    else:
        response = client.get("/me/has-password", headers={"Authorization": f"Bearer {data['token']}"})
...
def step_then_message(response_fixture, message) -> None:
    ...
    if response.status_code == 200:
        assert "has_password" in json_response
    else:
