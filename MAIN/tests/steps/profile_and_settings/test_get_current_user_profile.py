...
scenarios("../../features/profile_and_settings/get_current_user_profile.feature")
...
def step_when_reset_password(client, data, response_fixture) -> None:
    ...
    if data['token'] == None:
        response = client.get("/user/me/profile")
    else:
        response = client.get("/user/me/profile", headers={"Authorization": f"Bearer {data['token']}"})
...
def step_then_message(response_fixture, message) -> None:
    ...
    if response.status_code == 200:
        ...
        assert "profile_picture_url" in json_response
        assert "is_trainer" in json_response
        assert "registration_date" in json_response
    else:
