...
scenarios("../../features/profile_and_settings/update_name.feature")

not_decoded_jwts = tests_setup.create_not_decoded_iter_jwts("access")

@given(parsers.parse("{token} and {name}"))
def step_given_reset_password(token, name, data):
...
def step_when_reset_password(client, data, response_fixture):
    ...
    if data['token'] == None:
        response = client.put("/name", json=json_data)
    else:
        response = client.put("/name", json=json_data, headers={"Authorization": f"Bearer {data['token']}"})
