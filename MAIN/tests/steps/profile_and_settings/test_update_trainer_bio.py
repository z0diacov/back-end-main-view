...
scenarios("../../features/profile_and_settings/update_trainer_bio.feature")
...
def step_when_reset_password(client, data, response_fixture):
    ...
    if data['token'] == None:
        response = client.put("/trainer-bio", json=json_data)
    else:
        response = client.put("/trainer-bio", json=json_data, headers={"Authorization": f"Bearer {data['token']}"})
