...
scenarios("../../features/sessions/my_active_sessions.feature")
...
def step_when_reset_password(client, data, response_fixture) -> None:
    ...
    if data['token'] == None:
        response = client.get("/my-active-sessions")
    else:
        response = client.get("/my-active-sessions", headers={"Authorization": f"Bearer {data['token']}"})
